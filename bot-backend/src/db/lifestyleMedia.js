...
/**
 * Get lifestyle_media row with general_text for user and period (for duplicate check and summary).
 * @param {number} userId
...
export async function getByUserAndPeriod(userId, digestPeriodId) {
  const stmt = await db.prepare(
    'select * from lifestyle_media where user_id = ? and digest_period_id = ? and general_text is not null limit 1'
  );
...
export async function countPhotosByUserAndPeriod(userId, digestPeriodId) {
  const stmt = await db.prepare(
    'select count(*) as c from lifestyle_media where user_id = ? and digest_period_id = ? and media_url is not null'
  );
...
/**
 * Insert one lifestyle photo row (media_url only).
 * @param {number} userId
 * @param {number} digestPeriodId
...
export async function insertPhoto(userId, digestPeriodId, mediaUrl) {
  const stmt = await db.prepare(
    'insert into lifestyle_media (user_id, digest_period_id, media_url) values (?, ?, ?)'
  );
  stmt.run(userId, digestPeriodId, mediaUrl);
}

/**
 * Insert lifestyle general_text. No-op if row already exists (no duplicate).
 * @param {number} userId
 * @param {number} digestPeriodId
...
export async function insertGeneralText(userId, digestPeriodId, generalText) {
  ...
  const stmt = await db.prepare(
    'insert into lifestyle_media (user_id, digest_period_id, general_text) values (?, ?, ?)'
  );
...
/**
 * Get lifestyle photo rows (media_url not null) for user and period (for public API).
 * @param {number} userId
 * @param {number} digestPeriodId
...
export async function getPhotosByUserAndPeriod(userId, digestPeriodId) {
  const stmt = await db.prepare(
    'select media_url from lifestyle_media where user_id = ? and digest_period_id = ? and media_url is not null'
  );
